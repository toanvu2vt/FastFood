...
<body>
	<div class="container">
		<aside>
			...
			<div class="sidebar">
				<a href="/admin" id="dashboard" role="button"> <span
					...
					<h3>Dashboard</h3>
				</a> <a href="/admin/account" id="customers" role="button"> <span
					class="material-symbols-outlined">person</span>
					<h3>Account</h3>
				</a> <a href="/admin/product" id="products" role="button"> <span
		...
		<main>
			...
			<div id="productList" class="recent-orders"
				style="display: block; margin-bottom: 30px">
				<div id="productMana" style="display: block; margin-bottom: 10%">
					<h2>Product Management</h2>
					<form:form action="/admin/product" modelAttribute="pro">
						<c:if test="${not empty error_product}">
							<div class="form__group">
								<input
									style="background-color: #ffe4e4; border: 1px solid red; color: #b65a5a;"
									type="text" class="form__input" name="error"
									readonly="readonly" value="${error_product}" />
							</div>
						</c:if>
						<c:if test="${not empty success_product}">
							<div class="form__group">
								<input
									style="background-color: #e9ffe4; border: 1px solid #00ff1a; color: #63b65a;"
									type="text" class="form__input" name="success"
									readonly="readonly" value="${success_product}" />
							</div>
						</c:if>
						<div class="form__group">
							<form:input type="text" path="id" class="form__input" id="itemID"/>
						</div>
						<div class="form__group">
							<form:input type="text" path="name" class="form__input"
								name="name" placeholder="Name" />
						</div>
						 <div class="form__group">
							<form:input type="text" path="image" class="form__input"
								name="img" placeholder="images" />
						</div> 
						<div class="form__group">
							<form:input type="text" path="price" class="form__input"
								name="price" placeholder="price" />
						</div>
						<div class="form__group">
							<form:select path="category.id" class="form__input">
								<form:options items="${listCate}" itemValue="id" itemLabel="name"/>
								<%-- <c:forEach items="${listCate}" var="cate">
									<form:option value="${cate.id}">${cate.id}</form:option>
								</c:forEach> --%>
							</form:select>
						</div>

						<div class="form__group">
							<form:select path="active" class="form__input" name="active"
								id="Active">
								<c:if
									test="${pro.active == true || empty pro || pro.active == null}">
									<option value="true">Active</option>
									<option value="false">Stopped providing</option>
								</c:if>
								<c:if test="${pro.active == false}">
									<option value="true">Active</option>
						...
						<div class="buttons-container"
							...
							<button type="submit" class="button-arounder" formaction="/admin/product/delete/${id}">Delete</button>
							<button type="submit" class="button-arounder">New</button>
						</div>
					</form:form>
				</div>
					...
					<tbody>
						<c:forEach items="${products}" var="p">
							<tr>
								...
								<td class="primary"><a href="/admin/product/edit/${p.id}"><span
